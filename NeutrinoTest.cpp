...
double min_element(double array[]){
  ...
  if(n==1){
    printf("\n array size is 1, %d is both min and max",array[0]);
  }
  ...
  double max_element(double array[]){
...
while (done != true) {
 ...
 // Nold = ApplyPerturbation(Nold, amp dV, N_G); 
  ...
  BuildCollisionMatrix;
    ...
    for(int i=0; i < N_G; i++){
      rkVec[i] = pow(*kVec[i],-1.0);
    } 

  	dt_FE = min_element(rkVec);
